...
async function main() {
  ...
  // A compromised target can also leave its own password change uncommitted,
  // locking the pg_authid tuple needed by NOLOGIN. The managed fence must
  // drain that exact locker and keep draining while the administrator's
  // transition waits, so a reconnect cannot win the race or survive commit.
  stage = "target_password_locker_fence";
  const lockerProfile = profiles.find(profile => profile.name === "webhook");
  const lockerTarget = Object.freeze({ ...makeTarget(lockerProfile), roleOid: (await fixture.control.query(
    "SELECT oid::text value FROM pg_roles WHERE rolname=$1", [lockerProfile.role])).rows[0].value });
  const lockerNative = adapterModule.createLocalSyntheticProductionNativeAdapter({
  ...
  const reconnectPromise = (async () => {
    ...
    let reconnect;
    try {
      reconnect = await fixture.connect(lockerProfile.role, lockerCandidate.toString("ascii"), "tls");
      reconnectConnected = true;
      await reconnect.query("BEGIN");
      await reconnect.query("ALTER ROLE CURRENT_USER PASSWORD 'synthetic-reconnect-locker'");
  ...
  check(lockerReadback?.no_login === true && lockerReadback.no_inherit === true &&
    lockerReadback.membership_fenced === true && lockerReadback.sessions === 0 &&
    lockerReadback.verifier === verifierBefore,
