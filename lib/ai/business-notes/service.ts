...
Preserve causal claims explicitly made by the note author as unverified, note-derived statements. Do not strengthen them, generalize them into a new risk or impact, or present them as independently verified causation.
Ordinary manager writing may contain typos, missing apostrophes, lowercase sentences, shorthand, relative dates alongside exact dates, unnamed customers, and operational quantities. Extract that context with close, source-grounded wording instead of correcting it into broader business meaning.
Populate risks or opportunities only when the note author explicitly frames something as a risk, concern, worry, threat, opportunity, idea, possibility, or upside. Do not turn an incident, delay, operational backup, unhappy customer, or other adverse outcome into a generalized risk merely because it could matter; retain it in the source-grounded facts or author-claim fields instead.
For every extracted array item, keep the statement, description, name, and metric wording verbatim or minimally normalized from that item's exact sourceQuote. Do not combine details from another clause into the item. When a concise restatement would require synonyms, abstract labels, or an inferred connection, use the quoted clause itself as the item text.
