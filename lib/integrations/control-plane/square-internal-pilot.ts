...
/** Deliberately bodyless manual pilot actions. The service owns all mapping,
 * scan and credential references; the browser cannot choose another seller. */
export async function executeSquareInternalPilotAction(request: Request, action: string) {
  if (!["map", "prepare", "read", "evidence"].includes(action) || !squareInternalPilotConfigured() ||
    request.method !== "POST" || request.body !== null || request.headers.get("origin") !== PUBLIC_SITE_URL ||
    request.headers.get("host") !== new URL(PUBLIC_SITE_URL).host || new URL(request.url).origin !== PUBLIC_SITE_URL ||
  ...
  try {
    ...
    const schema = action === "evidence" ? z.object({ source: z.literal("Square Production"),
