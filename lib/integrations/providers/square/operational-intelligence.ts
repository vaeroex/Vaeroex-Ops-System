...
export function deriveSquareOperationalIntelligence(items:readonly SquareInterpretation[], relationships:readonly {fromFingerprint:string;state:string}[]) {
  ...
  const grouped=[...groups.values()].map(group=>{const first=group[0],value=moneyFor(first);return Group.parse({kind:first.kind,locationGroup:first.locationId?`sqg_${contractSha256({scope:first.scopeKey,location:first.locationId}).slice(7,23)}`:"explicitly_unresolved",currency:value?.currency??null,summary:summarizeSquareOperationalAmounts(group.map(moneyFor))});})
...
export function pageSquareOperationalActivity(intelligence:SquareOperationalIntelligence, input:{kind?:string;status?:string;page?:number;pageSize?:number}) {
  ...
  const rows=parsed.activity.filter(row=>(!input.kind||row.kind===input.kind)&&(!input.status||row.status===input.status));
