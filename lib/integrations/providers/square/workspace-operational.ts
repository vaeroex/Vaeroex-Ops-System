...
const KpiEvidence=z.object({evidenceRefs:z.array(z.string().regex(/^sqe_[a-f0-9]{16}$/)).max(1000),dimensionalScope:z.literal("current_workspace_entity_generation"),
  freshness:z.enum(["observed","stale","unknown","mixed"]),admission:z.literal("verified_non_economic"),completeness:z.literal("unknown"),reasonCodes:z.array(z.string().min(1).max(120)).min(1).max(8)}).strict();
export const SquareWorkspaceOperationalSchema=z.object({version:z.literal("square_workspace_operational_v1"),evidence:SquareWorkspaceEvidenceSchema,
  calculation:z.object({policyVersion:z.literal("square_operational_intelligence_v1"),economic:z.literal("blocked"),historical:z.literal("unknown"),aiDispatch:z.literal("disabled"),
    payment:Summary,refund:Summary,order:Summary,groups:z.array(z.object({kind:z.enum(["payment","refund","order"]),locationGroup:z.string().regex(/^(seller_scoped|explicitly_unresolved|sqg_[a-f0-9]{16})$/),currency:z.string().regex(/^[A-Z]{3}$/).nullable(),summary:Summary,evidenceRefs:z.array(z.string().regex(/^sqe_[a-f0-9]{16}$/)).min(1).max(1000)}).strict()).max(1000),refundRate:z.object({numerator:z.number().int().min(0),denominator:z.number().int().min(0),state:z.literal("observed_count_ratio_not_financial_rate")}).strict(),
