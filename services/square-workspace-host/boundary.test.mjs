...
assert(permitted(request('/activity')));
for (const route of ['/session','/signout','/activity-filter']) assert(permitted(request(route,'POST',['Host',HOST,'Origin',ORIGIN,'Content-Type','application/x-www-form-urlencoded','Content-Length','5'])));
...
for (const route of ['/session','/workspace','/signout','/activity-filter']) {
...
for (const route of ['/signout','/session','/workspace','/activity-filter']) assert.equal(permitted(request(route,'POST',['Host',HOST,'Origin',ORIGIN,'Content-Type','application/x-www-form-urlencoded','Content-Length','0'])),route==='/signout');
