...
gcloud run worker-pools update "$WORKER_POOL" \
  ...
  --region "$GCP_REGION" \
  --update-env-vars "DOCUMENT_EXTRACTION_PRIVATE_WORKER_ENABLED=true,DOCUMENT_EXTRACTION_PROVIDER_EXECUTION_ENABLED=false,DOCUMENT_EXTRACTION_BROKER_AUTH_QUALIFICATION_ENABLED=true,DOCUMENT_EXTRACTION_SYNTHETIC_QUALIFICATION_ENABLED=false,DOCUMENT_EXTRACTION_SYNTHETIC_PROVIDER_CALLS_ENABLED=false,DOCUMENT_EXTRACTION_RESPONSE_PROFILE_DIAGNOSTIC_ENABLED=false,DOCUMENT_EXTRACTION_FIELD_PATH_DIAGNOSTIC_ENABLED=false" \
