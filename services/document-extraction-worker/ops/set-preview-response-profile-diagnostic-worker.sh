...
: "${FIELD_PATH_DIAGNOSTIC_CONFIRMATION:?FIELD_PATH_DIAGNOSTIC_CONFIRMATION is required}"
...
if [ "$FIELD_PATH_DIAGNOSTIC_CONFIRMATION" != "nemotron-parse-field-path-one-call-v1" ]; then
  printf '%s\n' "One-call field-path diagnostic confirmation did not match." >&2
  exit 2
fi
