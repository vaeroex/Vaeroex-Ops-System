...
The first build step reads the build and trigger records through a read-only API role and rejects missing approval, storage/Git/connected-repository source substitution, a foreign trigger, repository, branch, config path, service account, or file scope, and any mismatch between the resolved repository revision and the exact 40-character commit. Images are tagged only with that commit, pushed by the dedicated builder, resolved to immutable digests, and held as non-deployable candidates until automatic Artifact Analysis discovery has completed successfully. Critical findings, new high findings, secret findings, incomplete scans, unreachable scan regions, and the accepted bootstrap exception without its exact source/dependency/compression fingerprints all fail the build. The builder can create a unique result only under `release-candidates/<commit>/<build-id>.json`; it cannot read, overwrite, or delete bucket objects.
