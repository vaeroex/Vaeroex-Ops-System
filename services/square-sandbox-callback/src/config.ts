...
import { checkedSquareGcpMappedBinding, squareGcpMappedHost } from "@/lib/integrations/control-plane/square-gcp-mapped-contracts";
...
const Schema = z.object({
  ...
  mappedBinding: z.unknown().optional(),
...
export function checkedPortalConfig(value: unknown, serving = false) {
  try {
    // One configuration + the two flat checked bindings (<=50 fields each).
    const raw = snapshotSquareDurableJson(value, { containers: 3, values: 120, bytes: 65_536,
      depth: 2, arrayLength: 0, properties: 50, stringLength: 2_048 });
    ...
    if (!config.enabled) {
      if (serving || config.binding !== null || config.supabasePublishableKey !== null ||
        config.databaseCaPath !== null || config.databaseCaSha256 !== null || config.mappedBinding !== undefined) throw new Error();
      return Object.freeze({ ...config, binding: null });
    }
    const binding = checkedSquareGcpCallbackBinding(config.binding);
    const mappedBinding = config.mappedBinding === undefined ? undefined : checkedSquareGcpMappedBinding(config.mappedBinding);
    if (mappedBinding && (mappedBinding.capability !== "broker" ||
      canonicalContractJson(squareGcpMappedHost(mappedBinding)) !== canonicalContractJson(binding))) throw new Error();
    ...
    return Object.freeze({ ...config, binding, ...(mappedBinding ? { mappedBinding } : {}) });
