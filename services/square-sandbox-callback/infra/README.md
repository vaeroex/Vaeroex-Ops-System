...
- Supported exact dated non-Pro Ubuntu 24.04 x86_64 image, one explicit Oregon zone (`us-west1-a`, `us-west1-b` or `us-west1-c`), patched Node version and artifact digest. No floating image family or implicit latest dependency. Zone compatibility does not authorize a replacement: fence the old runtime, verify the new numeric instance ID and signed identity, and update the canonical binding before credential use.

### Controlled zonal recovery

Do not apply a zone change to the existing Terraform instance: preserve its stopped VM and disk for recovery. A separately authorized replacement can retain the zonal instance name `square-sandbox-callback`, regional static IP, service account, callback hostname, broker and immutable encrypted credential version. Prepare a reviewed encrypted snapshot/disk copy within `us-west1`; account for temporary storage and remove only explicitly authorized recovery artifacts. Copying a disk also copies TLS/ACME and host identity material; never export these values. Recreate the reviewed instance metadata and verify OS Login, SSH host identity, guest privacy settings, timers and service inactivity after boot. Restore the inherited disabled configuration before fresh runtime approval.

Apply the additive Oregon-zone constraint migration before a guarded, closed-gate update of only the canonical zone and numeric instance ID. Preserve the existing approval expiry, tenant mapping, credential references and all other fields; require exact old-record matching and one affected row. Never change identity checks to accept the old VM or any arbitrary host. The original VM must stay stopped while the shared service account has temporary runtime access.

For an approved non-Spot recovery session, use a compatible machine family with `TERMINATE`, automatic restart disabled, `STOP`, a fresh provider-enforced stop timestamp and cleanup reserve. Do not silently use Standard E2, which requires live migration, or extend a failed window. Attempt capacity once per approved alternative zone, without reservations or commitments. Verify native binding, TLS and restricted ingress before private consent; successful consent still stops at `authorized_unmapped`, with enrollment/ingestion/economics disabled. Retain actual resource IDs, hashes, window outcomes and rollback instructions in the private operational handoff, not Terraform state or application logs.
