...
gcloud run deploy "$BROKER_SERVICE" \
  ...
  --labels "vaeroex-phase=phase-c1,vaeroex-pr=265,vaeroex-environment=preview" \
  --set-env-vars "DOCUMENT_EXTRACTION_BROKER_RUNTIME_ENVIRONMENT=preview,DOCUMENT_EXTRACTION_PRIVATE_WORKER_ENABLED=false,DOCUMENT_EXTRACTION_PROVIDER_EXECUTION_ENABLED=false,DOCUMENT_EXTRACTION_SYNTHETIC_QUALIFICATION_ENABLED=false,DOCUMENT_EXTRACTION_SYNTHETIC_PROVIDER_CALLS_ENABLED=false,DOCUMENT_EXTRACTION_ACTIVE_PROVIDER_PROFILE=hosted_tool_call_v2,DOCUMENT_EXTRACTION_NVIDIA_MODEL=nvidia/nemotron-parse,DOCUMENT_EXTRACTION_NVIDIA_CLIENT_REVISION=vaeroex_nemotron_parse_rest_v2,DOCUMENT_EXTRACTION_NVIDIA_PARSER_REVISION=nemotron_parse_hosted_tool_call_rest_v2,DOCUMENT_EXTRACTION_BROKER_CAPABILITY_CURRENT_KEY_VERSION=broker-capability-pr265-v1,DOCUMENT_EXTRACTION_ENCRYPTION_CURRENT_KEY_VERSION=cache-encryption-pr265-v1,NEXT_PUBLIC_SUPABASE_URL=$PREVIEW_SUPABASE_URL" \
...
printf '%s\n' "Ephemeral Preview broker deployed inert with exact service-level invocation."
