...
async function metadata(path: string) {
  ...
  if (!response.ok) throw new Error("qbo_production_google_metadata_failed");
...
async function accessToken() {
  ...
  if (typeof parsed.access_token !== "string" || parsed.access_token.length < 16) {
    throw new Error("qbo_production_google_access_token_failed");
  }
...
async function googleJson(url: string, init: RequestInit) {
  ...
  if (!response.ok) throw new Error("qbo_production_google_api_failed");
...
export async function googleCreateCloudTask(input: {
  ...
  if (
    !/^projects\/[a-z][a-z0-9-]{0,62}\/locations\/[a-z][a-z0-9-]{0,62}\/queues\/[a-z][a-z0-9-]{0,62}$/.test(input.queueResource) ||
    !/^[a-f0-9]{64}$/.test(input.taskId) ||
    !/^[a-z][a-z0-9-]{4,28}@[a-z][a-z0-9-]{0,62}\.iam\.gserviceaccount\.com$/.test(input.oidcServiceAccountEmail)
  ) {
    throw new Error("qbo_production_cloud_task_configuration_invalid");
  }
  ...
  if (
    ...
    throw new Error("qbo_production_cloud_task_target_invalid");
  }
  ...
  if (response.status === 409) return { taskName, created: false as const };
  if (!response.ok) throw new Error("qbo_production_cloud_task_create_failed");
  const result = await response.json() as { name?: unknown };
  if (result.name !== taskName) {
    throw new Error("qbo_production_cloud_task_create_response_invalid");
  }
  return { taskName, created: true as const };
}

export const googleCloudKmsTransport = {
  async encrypt(request: { name: string; plaintext: string; additionalAuthenticatedData: string }) {
    const result = await googleJson(`https://cloudkms.googleapis.com/v1/${request.name}:encrypt`, {
      method: "POST",
      body: JSON.stringify({
  ...
  async decrypt(request: { name: string; ciphertext: string; additionalAuthenticatedData: string }) {
    const result = await googleJson(`https://cloudkms.googleapis.com/v1/${request.name}:decrypt`, {
      method: "POST",
      body: JSON.stringify({
