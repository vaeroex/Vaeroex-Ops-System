...
-- Repair approvals created by the request-only workflow, but do not reactivate
-- an account that already has any subscription or billing history.
do $manual_activation_backfill$
declare
  request_to_repair record;
begin
  for request_to_repair in
    select request.id, request.reviewed_by
    from public.manual_activation_requests as request
    where request.status = 'approved'
      ...
      and not exists (
        select 1
        from public.customer_subscriptions as subscription
        where lower(subscription.customer_email) = lower(request.email)
      )
    order by request.reviewed_at, request.id
  loop
    perform public.review_manual_activation_request(
      request_to_repair.id,
      'approved',
      request_to_repair.reviewed_by,
      'vaeroex'
    );
  end loop;
end;
$manual_activation_backfill$;
