...
begin;
create function private.square_workspace_operational_v1(p_workspace_name text,p_kind text,p_status text,p_page integer)
...
begin
  if p_page not between 1 and 40 or (p_kind is not null and p_kind not in ('payment','refund','order','catalog','inventory'))
    or (p_status is not null and (length(p_status) not between 1 and 80 or p_status!~'^[A-Za-z0-9_-]+$')) then return null; end if;
...
revoke all on function private.square_workspace_operational_v1(text,text,text,integer) from public,anon,authenticated,service_role;
grant execute on function private.square_workspace_operational_v1(text,text,text,integer) to authenticated;
create function public.read_square_workspace_operational_v1(p_workspace_name text,p_kind text default null,p_status text default null,p_page integer default 1)
returns jsonb language sql security invoker set search_path=''
begin atomic; select private.square_workspace_operational_v1(p_workspace_name,p_kind,p_status,p_page); end;
revoke all on function public.read_square_workspace_operational_v1(text,text,text,integer) from public,anon,authenticated,service_role;
grant execute on function public.read_square_workspace_operational_v1(text,text,text,integer) to authenticated;
commit;
