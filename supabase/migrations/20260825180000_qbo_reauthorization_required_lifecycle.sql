-- Phase 8B recovery reauthorization lifecycle convergence.
--
-- First authorization remains pending_authorization-only. Existing
-- same-generation recovery from initializing remains unchanged. This migration
-- adds a separately evidenced path for a QBO sandbox connection and credential
-- that have already failed closed into reauthorization_required.

begin;

alter table private.integration_reauthorization_states
  add column reauthorization_path text not null
    default 'initializing_same_generation'
    check (
      reauthorization_path in (
        'initializing_same_generation',
        'authorization_required_recovery'
      )
    );

alter table private.integration_reauthorization_states
  alter column reauthorization_path drop default;

create or replace function
private.is_qbo_sandbox_recovery_reauthorization_eligible_v1(
  p_workspace_id uuid,
  ...
  p_credential_created_at timestamptz
)
returns boolean
language sql
stable
...
as $function$
  select exists (
    ...
    where audit.workspace_id = p_workspace_id
      and audit.business_entity_id = p_business_entity_id
      and audit.connection_id = p_connection_id
      and audit.action = 'reauthorization_required'
      and audit.outcome = 'failed'
      and audit.target_type = 'integration_credential'
      and audit.target_id = p_credential_id::text
      and audit.reason_code in (
        'credential_expired', 'scope_loss', 'kms_failure', 'integrity_failure'
      )
      and audit.metadata ->> 'connection_generation' =
        p_connection_generation::text
      and audit.metadata ->> 'credential_status' =
        'reauthorization_required'
      and audit.metadata ->> 'credential_version' =
        p_credential_version::text
      and audit.occurred_at >= p_credential_created_at
  ) and not exists (
    select 1
    from private.integration_audit_events as audit
    where audit.workspace_id = p_workspace_id
...
begin
  ...
  if v_state.status in ('consumed', 'completed') then
    ...
    where connection.workspace_id = v_state.workspace_id
      ...
      and connection.status = case v_state.reauthorization_path
        when 'initializing_same_generation' then 'initializing'
        else 'reauthorization_required'
      end
      and connection.state_reason_code = case v_state.reauthorization_path
      ...
      and connection.requested_scopes = v_state.requested_scopes
      and connection.granted_scopes = v_state.requested_scopes
      and credential.provider_key = v_state.provider_key
      and credential.provider_environment = v_state.provider_environment
      and credential.status = case v_state.reauthorization_path
      ...
      and credential.refresh_lease_owner_fingerprint is null
      and credential.refresh_lease_acquired_at is null
      ...
      and (
        (
          v_state.reauthorization_path = 'initializing_same_generation'
          and (
            select pg_catalog.count(*)
            from private.integration_sync_task_recovery_events as recovery
            where recovery.workspace_id = credential.workspace_id
              and recovery.business_entity_id = credential.business_entity_id
              and recovery.connection_id = credential.connection_id
              and recovery.connection_generation =
                credential.connection_generation
              and recovery.credential_id = credential.id
              and recovery.credential_version = credential.credential_version
          ) >= v_state.recovery_evidence_count
        )
        or (
          v_state.reauthorization_path = 'authorization_required_recovery'
          and private.is_qbo_sandbox_recovery_reauthorization_eligible_v1(
            credential.workspace_id,
            credential.business_entity_id,
...
revoke all on function
  private.is_qbo_sandbox_recovery_reauthorization_eligible_v1(
    uuid, uuid, uuid, bigint, uuid, bigint, timestamptz
  )
...
begin
  ...
  if v_connection.status = 'initializing'
    and v_connection.state_reason_code = 'initial_sync_pending' then
    v_reauthorization_path := 'initializing_same_generation';
  elsif v_connection.status = 'reauthorization_required'
    and v_connection.state_reason_code = 'authorization_required' then
    v_reauthorization_path := 'authorization_required_recovery';
  else
    raise exception using
      errcode = '42501',
      message = 'integration_reauthorization_state_connection_denied';
  ...
  where credential.workspace_id = v_connection.workspace_id
    ...
    and credential.status = case v_reauthorization_path
      when 'initializing_same_generation' then 'active'
      else 'reauthorization_required'
    end
    and credential.credential_ciphertext is not null
  ...
  if v_reauthorization_path = 'authorization_required_recovery'
    and not private.is_qbo_sandbox_recovery_reauthorization_eligible_v1(
  ...
  if v_reauthorization_path = 'initializing_same_generation' then
...
begin
  ...
  if found then
    if v_new_credential.last_request_id = p_request_id
      ...
      return pg_catalog.jsonb_build_object(
        ...
        'supersededCredentialVersion',
          v_new_credential.credential_version - 1,
  ...
  if not found
    ...
    or v_old_credential.refresh_lease_owner_fingerprint is not null
    or v_old_credential.refresh_lease_acquired_at is not null
    or v_old_credential.refresh_lease_expires_at is not null then
    raise exception using
      errcode = '40001',
      message = 'integration_credential_reauthorization_credential_stale';
  end if;

  if v_state.reauthorization_path = 'authorization_required_recovery'
    and not private.is_qbo_sandbox_recovery_reauthorization_eligible_v1(
      v_old_credential.workspace_id,
      v_old_credential.business_entity_id,
      v_old_credential.connection_id,
      v_old_credential.connection_generation,
      v_old_credential.id,
      v_old_credential.credential_version,
      v_old_credential.created_at
    ) then
    raise exception using
      errcode = '42501',
      message = 'integration_credential_reauthorization_credential_denied';
  end if;
  ...
  if v_state.reauthorization_path = 'authorization_required_recovery' then
    update private.integration_connections as connection
    set status = 'initializing',
        state_reason_code = 'initial_sync_pending',
...
as $function$
declare
  v_reauthorization_completion boolean := false;
begin
  ...
  if old.status = 'reauthorization_required'
    and old.state_reason_code = 'authorization_required'
    and new.status = 'initializing'
    and new.state_reason_code = 'initial_sync_pending' then
    select exists (
      select 1
      from private.integration_reauthorization_states as state
      join private.integration_credentials as superseded
        on superseded.id = state.superseded_credential_id
        and superseded.workspace_id = state.workspace_id
        and superseded.business_entity_id = state.business_entity_id
        and superseded.connection_id = state.connection_id
        and superseded.connection_generation = state.connection_generation
      join private.integration_credentials as replacement
        on replacement.id = state.replacement_credential_id
        and replacement.workspace_id = state.workspace_id
        and replacement.business_entity_id = state.business_entity_id
        and replacement.connection_id = state.connection_id
        and replacement.connection_generation = state.connection_generation
      where state.workspace_id = old.workspace_id
        and state.business_entity_id = old.business_entity_id
        and state.connection_id = old.id
        and state.connection_generation = old.connection_generation
        and state.provider_key = old.provider_key
        and state.provider_environment = old.provider_environment
        and state.reauthorization_path = 'authorization_required_recovery'
        and state.status = 'completed'
        and state.expected_connection_row_version = old.row_version
        and state.provider_entity_reference_fingerprint =
          old.provider_tenant_reference_fingerprint
        and state.requested_scopes = old.granted_scopes
        and state.completion_request_id = new.last_transition_request_id
        and state.completion_request_fingerprint =
          new.last_transition_request_fingerprint
        and state.completed_at = new.status_changed_at
        and state.completed_at = new.updated_at
        and superseded.status = 'superseded'
        and superseded.credential_version =
          state.superseded_credential_version
        and superseded.external_entity_reference_fingerprint =
          state.provider_entity_reference_fingerprint
        and replacement.status = 'active'
        and replacement.reauthorization_state_id = state.id
        and replacement.supersedes_credential_id = superseded.id
        and replacement.credential_version =
          state.superseded_credential_version + 1
        and replacement.granted_scopes = state.requested_scopes
        and replacement.external_entity_reference_fingerprint =
          state.provider_entity_reference_fingerprint
    ) into v_reauthorization_completion;

    if v_reauthorization_completion
      and (
        new.provider_tenant_reference_fingerprint is distinct from
          old.provider_tenant_reference_fingerprint
      ...
      v_reauthorization_completion := false;
    end if;
  end if;

  if old.status = new.status
    or (
      not v_reauthorization_completion
      and not private.is_integration_connection_transition_v1(
        old.status,
        new.status
      )
    ) then
  ...
  if new.id <> old.id
    ...
    or new.provider_descriptor_registry_version <>
      old.provider_descriptor_registry_version
    or new.provider_descriptor_registry_fingerprint <>
      old.provider_descriptor_registry_fingerprint
    or new.provider_descriptor_fingerprint <>
      old.provider_descriptor_fingerprint
