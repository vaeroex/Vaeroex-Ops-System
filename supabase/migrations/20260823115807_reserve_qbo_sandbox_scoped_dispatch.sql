-- Reserve a QBO sandbox dispatch under the current trusted connection scope
-- before any external Cloud Task is created.
...
begin
  ...
  where run.workspace_id = v_task.workspace_id
    and run.business_entity_id = v_task.business_entity_id
    and run.connection_id = v_task.connection_id
    and run.connection_generation = v_task.connection_generation
    and run.id = v_task.sync_run_id
