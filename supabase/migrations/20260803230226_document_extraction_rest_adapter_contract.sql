-- Document Extraction Phase B minimal REST-adapter contract
--
-- This forward-only migration changes the exact provider contract accepted by
-- the existing gated worker claim and dispatch functions. It creates no data,
-- enables no workspace, and changes no system or workspace switch.
...
comment on function public.claim_document_extraction_job_v2(text, integer) is
  'Claims one gated job matching the approved minimal Nemotron Parse REST adapter contract.';
...
begin
  ...
  if v_job.stage <> 'dispatching'
    ...
    or v_job.parser_revision <> 'nemotron_parse_hosted_tool_call_rest_v1'
    or v_job.client_revision <> 'vaeroex_nemotron_parse_rest_v1'
...
comment on function public.authorize_document_extraction_dispatch_v2(uuid, text, uuid) is
  'Single-use provider dispatch claim for the approved minimal Nemotron Parse REST adapter contract.';
