...
declare
  ...
  v_settings public.document_extraction_workspace_settings%rowtype;
begin
  if auth.uid() is null or auth.uid() <> p_requested_by or not public.is_workspace_member(p_workspace_id) then
    raise exception 'Workspace access denied.' using errcode = '42501';
  end if;
  if not exists (
  ...
  if p_content_hmac !~ '^[0-9a-f]{64}$' or p_cache_key !~ '^[0-9a-f]{64}$' then
    raise exception 'Invalid extraction identity.' using errcode = '22023';
  end if;
  if p_pages_qualified < 1 or p_pages_qualified > 10000 or p_max_attempts < 1 or p_max_attempts > 5 then
    raise exception 'Invalid extraction bounds.' using errcode = '22023';
  end if;

  perform pg_advisory_xact_lock(hashtextextended(p_workspace_id::text || ':' || p_cache_key, 0));

  select * into v_job from public.document_extraction_jobs where request_id = p_request_id for update;
  if v_job.id is not null then
    if v_job.workspace_id <> p_workspace_id or v_job.file_id <> p_file_id or v_job.cache_key <> p_cache_key then
      raise exception 'Idempotency replay does not match the original request.' using errcode = '23505';
    end if;
    update public.file_uploads
    set metadata_json = coalesce(metadata_json, '{}'::jsonb) || jsonb_build_object(
      'document_extraction_job_id', v_job.id::text,
      'document_extraction_authority', coalesce(
        (
          select jsonb_build_object(
            'job_id', v_job.id::text,
            'review_id', review.id::text,
            'artifact_fingerprint', review.artifact_fingerprint,
            'classification_fingerprint', review.classification_fingerprint,
            'review_version', review.review_version
          )
          from public.document_extraction_reviews review
          where review.workspace_id = p_workspace_id
            and review.job_id = v_job.id
            and review.status in ('approved', 'approved_with_corrections')
            and review.review_version = v_job.required_review_version
          limit 1
        ),
        jsonb_build_object('job_id', v_job.id::text)
      )
    )
  ...
  for update;
  if v_job.id is not null then
    insert into public.document_extraction_file_bindings (workspace_id, file_id, job_id, cache_key, created_by)
    values (p_workspace_id, p_file_id, v_job.id, p_cache_key, p_requested_by)
    on conflict (workspace_id, file_id, job_id) do nothing;
    update public.file_uploads
    set metadata_json = coalesce(metadata_json, '{}'::jsonb) || jsonb_build_object(
      'document_extraction_job_id', v_job.id::text,
      'document_extraction_authority', coalesce(
    ...
    if v_job.status = 'completed' then
      ...
      where workspace_id = p_workspace_id and cache_key = p_cache_key and invalidated_at is null;
    end if;
    return jsonb_build_object(
      'job_id', v_job.id, 'status', v_job.status, 'idempotent', true,
      'cache_hit', exists (
        select 1 from public.document_extraction_cache cache
        where cache.workspace_id = p_workspace_id and cache.cache_key = p_cache_key and cache.invalidated_at is null
      )
    );
  end if;
  ...
  v_reason := public.document_extraction_runtime_reason_v1(p_workspace_id, p_document_class, p_pages_qualified);
  ...
  update public.document_extraction_workspace_settings
  set pages_reserved = pages_reserved + p_pages_qualified,
      updated_at = now()
  where workspace_id = p_workspace_id;

  insert into public.document_extraction_jobs (
    workspace_id, file_id, requested_by, request_id, route, document_class,
    parser_provider, parser_model, parser_revision, client_revision, content_hmac,
    cache_key, routing_policy_version, extraction_contract_version,
    normalization_version, pages_qualified, reserved_page_count, max_attempts,
    review_required, approval_status
  ) values (
    p_workspace_id, p_file_id, p_requested_by, p_request_id, p_route, p_document_class,
    p_parser_provider, p_parser_model, p_parser_revision, p_client_revision, p_content_hmac,
    p_cache_key, p_routing_policy_version, p_extraction_contract_version,
    p_normalization_version, p_pages_qualified, p_pages_qualified, p_max_attempts,
  ...
  set metadata_json = coalesce(metadata_json, '{}'::jsonb) || jsonb_build_object(
    'document_extraction_job_id', v_job.id::text,
    'document_extraction_authority', jsonb_build_object('job_id', v_job.id::text)
  )
  where id = p_file_id and workspace_id = p_workspace_id;
  perform public.record_document_extraction_event_v1(
    p_workspace_id, v_job.id, 'job_enqueued', 'user', p_requested_by, v_job.stage,
    v_job.status, null, null,
    jsonb_build_object('route', p_route, 'document_class', p_document_class, 'pages_reserved', p_pages_qualified),
    p_request_id
  );
...
begin
  ...
  if p_page_count < 1 or p_page_count > 10000
  ...
  if v_existing.id is null then
    insert into public.document_extraction_cache (
      ...
      p_aad_digest, p_artifact_fingerprint, p_page_count
    );
  end if;
  ...
  perform public.record_document_extraction_event_v1(
    ...
    v_job.artifact_fingerprint, jsonb_build_object('page_count', p_page_count), gen_random_uuid()
  );
...
create or replace function public.mutate_document_extraction_review_v1(
  ...
  p_critical_field_count integer,
  p_confirmed_field_count integer,
  p_corrected_field_count integer,
  p_rejected_field_count integer,
  p_unresolved_field_count integer,
...
begin
  ...
  if p_status not in ('in_review', 'approved', 'approved_with_corrections', 'rejected', 'unresolved') then
  ...
  if p_decision_summary_json is null or jsonb_typeof(p_decision_summary_json) <> 'object'
    or octet_length(p_decision_summary_json::text) > 32768 then
    raise exception 'Invalid bounded review decision.' using errcode = '22023';
  end if;
  if least(p_critical_field_count, p_confirmed_field_count, p_corrected_field_count, p_rejected_field_count, p_unresolved_field_count) < 0
    or p_confirmed_field_count + p_corrected_field_count + p_rejected_field_count + p_unresolved_field_count <> p_critical_field_count then
    raise exception 'Invalid critical-field counts.' using errcode = '22023';
  end if;
  ...
  if p_review_version <> v_job.required_review_version then
    raise exception 'Review contract version is stale.' using errcode = '22023';
  end if;
  if p_status in ('approved', 'approved_with_corrections') and (
    p_critical_field_count = 0 or p_rejected_field_count > 0 or p_unresolved_field_count > 0
  ...
  insert into public.document_extraction_reviews (
    ...
    artifact_fingerprint, classification_fingerprint, review_version,
    ...
    p_workspace_id, p_job_id, p_file_id, p_status, v_actor,
    case when p_status in ('approved', 'approved_with_corrections', 'rejected') then now() else null end,
    p_artifact_fingerprint, p_classification_fingerprint, p_review_version,
    p_critical_field_count, p_confirmed_field_count, p_corrected_field_count,
    p_rejected_field_count, p_unresolved_field_count, p_decision_summary_json
  )
  ...
  set approval_status = p_status,
      status = case when p_status in ('approved', 'approved_with_corrections') then 'completed' else status end,
      stage = case when p_status in ('approved', 'approved_with_corrections') then 'terminal' else 'awaiting_review' end,
      updated_at = now()
  where id = p_job_id returning * into v_job;
  if p_status in ('approved', 'approved_with_corrections') then
    update public.file_uploads file
    set metadata_json = coalesce(file.metadata_json, '{}'::jsonb) || jsonb_build_object(
      'document_extraction_job_id', p_job_id::text,
    ...
    where file.workspace_id = p_workspace_id
      and exists (
        ...
        where binding.workspace_id = p_workspace_id
          ...
          and binding.file_id = file.id
      );
  end if;
  perform public.record_document_extraction_event_v1(
    p_workspace_id, p_job_id,
    case when p_status in ('approved', 'approved_with_corrections') then 'review_approved' else 'review_updated' end,
    'user', v_actor, v_job.stage, v_job.status, p_status, p_artifact_fingerprint,
    jsonb_build_object(
      'review_id', v_review.id, 'review_version', p_review_version,
      'critical_fields', p_critical_field_count, 'confirmed_fields', p_confirmed_field_count,
      'corrected_fields', p_corrected_field_count, 'rejected_fields', p_rejected_field_count,
      'unresolved_fields', p_unresolved_field_count
    ), p_request_id
  );
  return jsonb_build_object('review_id', v_review.id, 'job_id', p_job_id, 'status', p_status);
end;
$$;
...
as $$
  select exists (
    ...
    join public.document_extraction_file_bindings binding
      on binding.workspace_id = job.workspace_id and binding.job_id = job.id and binding.file_id = p_file_id
...
begin
  ...
  if v_file_id is null or not exists (
    select 1 from public.document_extraction_file_bindings binding
    where binding.workspace_id = new.workspace_id and binding.file_id = v_file_id
  ) then
    return new;
  end if;
...
revoke execute on function public.enqueue_document_extraction_job_v1(uuid, uuid, uuid, uuid, text, text, text, text, text, text, text, text, text, text, text, integer, integer) from public, anon, authenticated, service_role;
...
revoke execute on function public.complete_document_extraction_job_v1(uuid, text, integer, text, text, bytea, text, bytea, bytea, text) from public, anon, authenticated, service_role;
...
revoke execute on function public.mutate_document_extraction_review_v1(uuid, uuid, uuid, text, text, text, integer, integer, integer, integer, integer, integer, jsonb, uuid) from public, anon, authenticated, service_role;
...
grant execute on function public.enqueue_document_extraction_job_v1(uuid, uuid, uuid, uuid, text, text, text, text, text, text, text, text, text, text, text, integer, integer) to authenticated;
grant execute on function public.mutate_document_extraction_review_v1(uuid, uuid, uuid, text, text, text, integer, integer, integer, integer, integer, integer, jsonb, uuid) to authenticated;
grant execute on function public.assert_document_extraction_authority_v1(uuid, uuid, uuid, uuid, text, text, integer) to authenticated, service_role;
...
grant execute on function public.complete_document_extraction_job_v1(uuid, text, integer, text, text, bytea, text, bytea, bytea, text) to service_role;
