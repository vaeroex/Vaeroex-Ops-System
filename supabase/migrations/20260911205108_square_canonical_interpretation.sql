...
create table private.square_interpretation_runs (
  ...
  retention_expires_at timestamptz not null check(isfinite(retention_expires_at)),
  created_at timestamptz not null default clock_timestamp(),
  primary key(connection_id,generation,revision), unique(connection_id,generation,input_fingerprint),
  foreign key(workspace_id,business_entity_id,connection_id,generation)
    references private.square_connection_generations(workspace_id,business_entity_id,connection_id,connection_generation)
);
...
declare a jsonb; m jsonb; checked jsonb; observed jsonb; inputs jsonb:='[]'; prior jsonb;
begin
  ...
  return jsonb_build_object('inputs',inputs,'prior',prior,'asOf',to_char(clock_timestamp() at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'));
end $$;
...
begin
  ...
  select input_fingerprint into prior_fingerprint from private.square_interpretation_runs
    where connection_id=(a->>'connectionId')::uuid and generation=(a->>'generation')::bigint and input_fingerprint=p_input_fingerprint;
  ...
  insert into private.square_interpretation_runs(connection_id,generation,revision,workspace_id,business_entity_id,approval_fingerprint,input_fingerprint,output,retention_expires_at)
    values((a->>'connectionId')::uuid,(a->>'generation')::bigint,revision+1,(a->>'workspaceId')::uuid,(a->>'businessEntityId')::uuid,p_approval_fingerprint,p_input_fingerprint,p_output,
