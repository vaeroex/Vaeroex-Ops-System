-- Separate global scheduling from connection-scoped dispatch and recover only
-- tasks inside the QBO sandbox dispatcher's trusted runtime boundary.
...
begin
  if not exists (
    ...
    where rolname = 'integration_task_scheduler_authority'
  ) then
    create role integration_task_scheduler_authority nologin noinherit;
  end if;
end;
$role$;

alter role integration_task_scheduler_authority nologin noinherit;
...
revoke all on function public.sweep_qbo_sandbox_scoped_dispatch_tasks_v1(
  jsonb, text, text
)
...
grant execute on function public.sweep_qbo_sandbox_scoped_dispatch_tasks_v1(
  jsonb, text, text
)
