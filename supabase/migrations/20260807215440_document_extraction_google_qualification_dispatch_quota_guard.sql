-- Preserve the merged claim-aware and processing-cleanup guard while allowing
-- only the exact reservation-bound workspace quota transition performed by the
-- qualified Google dispatch path. This forward fix changes no data, grants no
-- execution capability, and enables no provider path.
...
declare
  ...
  v_dispatch_job public.document_extraction_jobs%rowtype;
  v_dispatch_binding public.document_extraction_google_qualification_job_bindings%rowtype;
...
begin
  ...
  if v_environment.id is null or v_run.id is null
    ...
    or (
      tg_table_name in ('workspaces', 'workspace_members')
      and v_operation <> 'enqueue'
    )
    or (
      tg_table_name = 'document_extraction_workspace_settings'
      and v_operation not in ('enqueue', 'dispatch')
    )
    or (
      v_operation = 'enqueue'
      and (
  ...
  if tg_table_name = 'document_extraction_workspace_settings'
    and v_operation = 'dispatch' then
    select job.* into v_dispatch_job
    from public.document_extraction_jobs job
    where job.id = v_item.job_id;
    select binding.* into v_dispatch_binding
    from public.document_extraction_google_qualification_job_bindings binding
    where binding.job_id = v_item.job_id;

    if tg_op <> 'UPDATE'
      or v_context_job_id is null
      or v_context_job_id <> v_item.job_id
      or v_dispatch_job.id is null
      or v_dispatch_binding.id is null
      or v_run.status <> 'active'
      or v_run.stop_reason is not null
      or v_run.stopped_at is not null
      or v_run.active_fixture_index <> v_item.fixture_index
      or v_run.provider_reservation_count <> v_reservation.reservation_number
      or v_run.provider_reservation_count > v_run.provider_reservation_limit
      or v_run.provider_call_count >= v_run.provider_call_limit
      or v_run.retry_count <> 0
      or v_item.status <> 'processing'
      or v_item.job_id <> v_dispatch_job.id
      or v_item.route <> 'google_primary'
      or v_item.provider_reservation_count <> v_reservation.page_index
      or v_item.provider_call_count <> 0
      or v_dispatch_job.workspace_id <> v_run.workspace_id
      or v_dispatch_job.intake_request_id <> v_item.intake_request_id
      or v_dispatch_job.file_id <> v_item.file_id
      or v_dispatch_job.status <> 'processing'
      or v_dispatch_job.stage <> 'dispatching'
      or v_dispatch_job.attempts <> 1
      or v_dispatch_job.max_attempts <> 1
      or v_dispatch_job.retry_count <> 0
      or v_dispatch_job.route <> 'google_primary'
      or v_dispatch_job.lease_owner is null
      or v_dispatch_job.lease_owner <> v_reservation.worker_id
      or v_dispatch_job.lease_expires_at is null
      or v_dispatch_job.lease_expires_at <= now()
      or v_dispatch_job.lease_expires_at <> v_reservation.lease_expires_at
      or v_dispatch_job.provider_dispatched_at is not null
      or v_dispatch_job.dispatch_request_id is not null
      or v_dispatch_job.provider_call_count <> 0
      or v_dispatch_job.reserved_page_count <> v_dispatch_job.page_count
      or v_dispatch_job.reserved_page_count < 1
      or not public.document_extraction_google_job_identity_is_exact_v1(v_dispatch_job)
      or v_dispatch_binding.run_id <> v_run.id
      or v_dispatch_binding.item_id <> v_item.id
      or v_dispatch_binding.source_binding_id <> v_source.id
      or v_dispatch_binding.job_id <> v_dispatch_job.id
      or v_dispatch_binding.intake_request_id <> v_item.intake_request_id
      or v_dispatch_binding.file_id <> v_item.file_id
      or v_dispatch_binding.workspace_id <> v_run.workspace_id
      or v_dispatch_binding.fixture_index <> v_item.fixture_index
      or v_dispatch_binding.fixture_identity_fingerprint <>
        v_item.fixture_identity_fingerprint
      or v_dispatch_binding.source_sha256 <> v_item.source_sha256
      or v_dispatch_binding.page_identity_fingerprints <>
        v_item.page_identity_fingerprints
      or v_dispatch_binding.corpus_sha256 <> v_run.corpus_sha256
      or v_dispatch_binding.provider_profile <> v_run.provider_profile
      or v_dispatch_binding.processor_id <> v_run.processor_id
      or v_dispatch_binding.processor_resource <> v_run.processor_resource
      or v_dispatch_binding.processor_version <> v_run.processor_version
      or v_dispatch_binding.preview_project_ref <>
        v_environment.supabase_project_ref
      or v_dispatch_binding.controller_version <> v_run.controller_version
      or v_environment.environment <> 'preview'
      or v_environment.supabase_project_ref <> 'zfpnhvcmuuvtswttmnjd'
      or v_environment.production_project_ref_exclusion <>
        'mdiianhfrojmxqpwrflh'
      ...
      or v_reservation.status <> 'reserved'
      or v_reservation.dispatch_request_id is null
      or v_reservation.provider <> 'google_document_ai'
      or v_reservation.provider_profile <> v_run.provider_profile
      or v_reservation.processor_id <> v_run.processor_id
      or v_reservation.processor_resource <> v_run.processor_resource
      or v_reservation.processor_version <> v_run.processor_version
      or v_reservation.controller_version <> v_run.controller_version
      or old.workspace_id <> v_run.workspace_id
      or new.workspace_id <> old.workspace_id
      or old.pages_reserved < v_dispatch_job.reserved_page_count
      or new.pages_reserved <>
        greatest(0, old.pages_reserved - v_dispatch_job.reserved_page_count)
      or new.pages_consumed <>
        old.pages_consumed + v_dispatch_job.reserved_page_count
      or new.updated_at is distinct from now()
      or new.is_entitled is distinct from old.is_entitled
      or new.is_enabled is distinct from old.is_enabled
      or new.monthly_page_limit is distinct from old.monthly_page_limit
      or new.current_period_start is distinct from old.current_period_start
      or new.current_period_end is distinct from old.current_period_end
      or new.concurrent_job_limit is distinct from old.concurrent_job_limit
      or new.allowed_document_classes is distinct from old.allowed_document_classes
