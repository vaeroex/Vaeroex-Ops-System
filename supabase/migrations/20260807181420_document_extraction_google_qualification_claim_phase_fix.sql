-- Bind the qualification claim guard to the exact two-phase claim transaction.
--
-- This forward fix does not enable extraction. It replaces only the mutation
-- guard and qualification claim RPC so that the authoritative queued-to-leased
-- transition can be followed by one exact claimed event in the same transaction.
...
begin
  ...
  if v_environment.id is null
    or (v_context ->> 'run_id')::uuid <> v_run.id
    or (v_context ->> 'item_id')::uuid <> v_item.id
    or (v_context ->> 'job_id')::uuid <> v_job.id
    or v_context ->> 'operation' <> 'claim' then
    raise exception 'Google qualification claim context is not authorized.'
      using errcode = '42501';
  end if;
  v_context := v_context || jsonb_build_object(
    'claim_worker_id', p_worker_id,
    'claim_lease_seconds', p_lease_seconds
  );
...
comment on function public.claim_google_frozen_qualification_job_v1(text, integer) is
  'Claims exactly one controller-bound Google qualification job. The signed transaction context admits only the exact first lease transition and its single claimed event.';
