...
3. Apply the infrastructure-only plan first with no image digest. It may create empty secret containers but never secret versions. Build the disabled bootstrap as the dedicated build identity, pin its Artifact Registry digest, then apply the runtime/LB plan with every gate false.
...
5. Deploy by immutable digest with direct `run.app` ingress denied. Prove exact Host/SNI/routes, forwarded-authority rejection, OIDC audience/invoker, provider egress allowlist, database `session_user`, callback/webhook raw-body handling, sanitization, bounded concurrency/rate/retry, and zero transaction-level AI.
