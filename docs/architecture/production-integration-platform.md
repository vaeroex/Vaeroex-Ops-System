...
All platform and Square gates are structurally false in the activation layer. Its pinned bootstrap image serves only a static TLS health result and otherwise returns a disabled `404`; provider calls, onboarding, webhook intake, evidence, economics and AI cannot be enabled by configuration alone. Empty Secret Manager containers and exact version-1 IAM conditions do not create credentials. No individual Square transaction or raw provider payload is an AI input. Any later compact-snapshot AI route remains a separate, budgeted, deterministic eligibility milestone.
