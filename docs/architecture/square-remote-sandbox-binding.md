...
The first PR CI run passed `verify`, all existing database qualifications and the fixture-rich QBO upgrade, then exposed two native-superuser assumptions in the new test harness. A PostgreSQL 17.6 reproduction using a non-superuser administrator confirmed that automatic creator membership has `ADMIN` but not `SET`, and that `SUPERUSER` cannot be assigned by that administrator. Tests now distinguish engine-level denials from checked-function denials, without granting extra authority or changing any migration/runtime predicate. Native elevated-role mutation coverage remains, and the constrained-administrator regression exercises the managed permission profile. The corrected exact-head CI run remains the final delivery gate. [PostgreSQL role changes](https://www.postgresql.org/docs/17/sql-alterrole.html), [SET ROLE authority](https://www.postgresql.org/docs/17/sql-set-role.html).

After that test-only correction, both complete local qualifications pass **156 assertions / 75 scenarios**: native bootstrap superuser and a real constrained-administrator LOGIN in an isolated temporary test context. Independently restoring each original fragment reproduces its own `42501` failure, first at unavailable `SET ROLE` and then at `ALTER ROLE ... SUPERUSER`. Independent review cleared the exact corrected test SHA-256 `ea0600d00d7165aa4dbeb1f52b3c52f4d01bc7c2dc0f79df7c51e92d618a75a1`. The migration, runtime, ACLs, deployment exclusion and CI workflow are unchanged by this correction.
