...
Focused suites: OAuth/broker 244 assertions, discovery/mapping 119, routes 197. Combined actual PostgreSQL + synthetic-provider + browser qualification: 395 assertions across 60 scenarios. Existing durable qualification with all three migrations: 572 assertions across 226 scenarios. The new local-disconnect regression fails against the prior `f555549` implementation in a separate disposable worktree/database at its zero-application-secret-access assertion. These are local evidence, **not Square Sandbox qualification**. Final exact-head CI/Preview is reported with the delivery PR.
