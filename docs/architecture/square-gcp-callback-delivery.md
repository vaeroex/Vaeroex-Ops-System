...
- Ops: **99** synthetic/loopback assertions; Terraform 1.16.1 + Google 8.1.0 formatting/schema validation and **9 mocked-provider cases**. No real cloud plan/apply.
...
Post-opening automated review identified the missing `NODE_EXTRA_CA_CERTS` launch guard. The narrow correction rejects/removes ambient TLS/CA/OpenSSL/proxy overrides (including the database CA override) and permits only the reviewed Node argument before any configuration/network access. Independent in-memory restoration of the original guard reproduced its configuration-read reachability for extra roots, disabled TLS validation, proxy enablement and the DB CA override; the corrected guard rejected all four before the first intercepted read. An owned loopback sink independently confirmed Node 24 ambient proxy routing without forwarding any request. The updated nonbrowser suite independently passed **530** assertions; browser/ops, executable packaging, typecheck, lint, security and build checks were rerun. No remaining material finding in the corrected boundary; exact corrected-head CI is required before delivery. This changes no shared adapter, database routine or accepted provider-host residual risk.
