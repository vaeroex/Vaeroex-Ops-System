  ...
  return (
    <article className="rounded-lg border border-cyan-300/30 bg-cyan-950/15 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-cyan-200">Extraction review required</p>
      ...
      <form action={approveBusinessNoteAction} className="mt-4 space-y-5">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <PendingSubmitButton pendingLabel="Approving note..." className="min-h-11 rounded-md bg-vaeroex-blue px-4 py-2 text-sm font-semibold text-white">
            Approve extraction
          </PendingSubmitButton>
        </div>
      </form>
    ...
    <section id="business-notes" className="rounded-lg border border-white/10 bg-[#08111f] p-4 text-slate-100 shadow-panel sm:p-5">
      ...
      <details className="mt-4 rounded-lg border border-cyan-300/20 bg-cyan-950/10 p-3" open={!notes.length}>
        ...
        <form action={submitBusinessNoteForReviewAction} className="mt-4 space-y-3">
          ...
          <p className="text-xs leading-5 text-slate-400">Extraction runs only after submission. Documents and longer meeting notes should continue through the existing Evidence upload action.</p>
          {enabled ? (
            <PendingSubmitButton pendingLabel="Preparing extraction review..." className="min-h-11 rounded-md bg-vaeroex-blue px-4 py-2 text-sm font-semibold text-white">
              Add Note for Review
            </PendingSubmitButton>
          ) : (
            <p className="rounded-md border border-amber-300/20 bg-amber-950/15 p-3 text-sm text-amber-100">Business Notes extraction is not enabled in this environment.</p>
          )}
        </form>
      </details>
