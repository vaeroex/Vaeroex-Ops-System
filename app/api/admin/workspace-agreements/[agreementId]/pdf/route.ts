...
export async function GET(request: Request, { params }: { params: Promise<{ agreementId: string }> }) {
  ...
  if (!access.allowed || !access.admin) return NextResponse.json({ error: "Not found." }, { status: 404 });

  const { agreementId } = await params;
  const { data: agreement, error } = await access.admin
  ...
  const download = await access.admin.storage.from(agreement.storage_bucket).download(agreement.storage_path);
  if (download.error || !download.data) return NextResponse.json({ error: "Agreement PDF is unavailable." }, { status: 503 });

  const bytes = Buffer.from(await download.data.arrayBuffer());
  if (createHash("sha256").update(bytes).digest("hex") !== agreement.pdf_sha256) {
