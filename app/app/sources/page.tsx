...
function SourceActionButton({
  ...
  return (
    <AnalysisProgressSubmit
      className="rounded-md bg-vaeroex-blue px-3 py-2 text-xs font-semibold text-white disabled:opacity-50"
      pendingLabel={pendingLabel}
      steps={steps}
      persistedPending={persistedPending}
      showSteps={showSteps}
      longRunningMessage="Analysis is still running. You can keep this page open or return later; this source will continue to show its processing state."
    >
      {children}
    </AnalysisProgressSubmit>
  );
    ...
    <div className="flex flex-wrap gap-2">
      ...
      {status !== "Archived" && !isSpreadsheet(file) ? (
        <form action={analyzeFileAction}>
          ...
          />
          <SourceActionButton
            pendingLabel="Analyzing source..."
            steps={ANALYSIS_PROGRESS_STEPS}
            persistedPending={status === "Analyzing"}
            showSteps={false}
          >
            {status === "Uploaded"
              ? "Analyze source"
