...
function riskItems(source: Awaited<ReturnType<typeof fetchReportSource>>) {
  const risks = [
    source.counts.overdue_tasks ? `${source.counts.overdue_tasks} unresolved source-system signal${source.counts.overdue_tasks === 1 ? "" : "s"} need leadership review.` : "",
...
function recommendedActions(source: Awaited<ReturnType<typeof fetchReportSource>>) {
  const actions = [
    source.counts.overdue_tasks ? "Review the workflow behind unresolved source-system signals before the next management review." : "",
...
function buildReportBody({
  ...
  const summary =
    ...
    `${current.counts.overdue_tasks} unresolved source-system signal${current.counts.overdue_tasks === 1 ? "" : "s"} need attention.`;
...
## Overdue Follow-ups
${readableList(current.items.overdue_tasks, "No unresolved source-system signals were found for this period.")}
