...
function isDemoWorkspace(workspace?: { name?: string | null; subscription_status?: string | null } | null) {
...
async function findDemoWorkspace(supabase: SupabaseServerClient, userId: string) {
  ...
  return workspaces.find(isDemoWorkspace) ?? null;
}

async function findNonDemoWorkspace(supabase: SupabaseServerClient, userId: string) {
  const workspaces = await listUserWorkspaces(supabase, userId);
  return workspaces.find((workspace) => !isDemoWorkspace(workspace)) ?? null;
}

async function createSeededDemoWorkspace(supabase: SupabaseServerClient, user: SignedInUser, name = "Vaeroex Demo Workspace") {
  if (!user) {
...
export async function createDemoWorkspaceAction() {
  ...
  let destination: Route = appMessage("Demo workspace created with sample data");
  ...
  try {
    ...
    if (existing) {
      ...
      const workspaceId = await createSeededDemoWorkspace(supabase, user);
...
export async function openDemoWorkspaceAction() {
  ...
  redirect(appMessage("Demo workspace opened."));
}
...
export async function createFreshDemoWorkspaceAction() {
  ...
  const workspaceId = await createSeededDemoWorkspace(supabase, user, `Vaeroex Demo Workspace ${stamp}`);
  await setActiveWorkspace(workspaceId);
  redirect(appMessage("Fresh demo workspace created with sample data."));
}
...
export async function resetDemoWorkspaceAction() {
  ...
  const workspaceId = await createSeededDemoWorkspace(supabase, user);
  await setActiveWorkspace(workspaceId);
  redirect(appMessage("Demo workspace reset with sample data."));
}
