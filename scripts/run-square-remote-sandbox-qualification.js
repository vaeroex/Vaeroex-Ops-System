...
async function migrationTests(runtime) {
  ...
  equal(files.filter(name => name === productionFoundation).length, 1,
    "provider-neutral Production foundation remains separately qualified from the remote Sandbox binding");
  equal(files.filter(name => name === productionOverlay).length, 1,
    "Production-only Square overlay remains excluded from the remote Sandbox binding");
  equal(files.filter(name => name === productionInternalRuntime).length, 1,
    "Production internal-pilot runtime remains separately qualified from the remote Sandbox binding");
