...
assert.match(databaseRunner, /host=\$\{quoteConnectionValue\(host\)\}[\s\S]+port='5432'[\s\S]+sslmode='disable'/, "local dblink sessions must use the password-authenticated container network rather than the host-facing Supabase port or a trusted socket");
