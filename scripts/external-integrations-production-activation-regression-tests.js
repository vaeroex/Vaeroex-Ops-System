...
assert.match(edgePlugin, /diagnosticCanaryTarget\s*=\s*callbackedge\.CallbackPath \+ "\?state=A{43}&code=VAEROEX_PUBLIC_NEVER_ISSUED_CANARY"/, "the temporary diagnostic recognizes only the existing exact public synthetic canary");
assert.match(edgePlugin, /OnPluginStart[\s\S]*GetPluginConfiguration\(\)[\s\S]*parseDiagnosticWindow/, "the temporary diagnostic is disabled without explicit reviewed plugin configuration");
assert.match(edgePlugin, /maxDiagnosticWindowSeconds\s*=\s*int64\(1200\)/, "the temporary diagnostic cannot exceed twenty minutes");
assert.match(edgePlugin, /!window\.enabledAt\(nowUnix\)/, "the temporary diagnostic rejects requests outside its exact configured window");
assert.match(edgePlugin, /return status, "callback_predicate_" \+ string\(reason\), true/, "the temporary diagnostic emits only one fixed parser-predicate label");
assert.doesNotMatch(edgePlugin, /proxywasm\.Log|fmt\.(?:Print|Printf|Println)|log\./, "the temporary diagnostic emits no request or callback logs");
assert.doesNotMatch(edgePlugin, /1789\d{6}/, "no active diagnostic timestamps are compiled into the callback edge");
assert.match(variables, /variable "callback_edge_diagnostic_window"[\s\S]*expires_unix - var\.callback_edge_diagnostic_window\.not_before_unix <= 1200/, "Terraform rejects any diagnostic window longer than twenty minutes");
assert.match(main, /callback_edge_config_suffix[\s\S]*substr\(sha256\(local\.callback_edge_diagnostic_config\), 0, 12\)/, "the immutable Wasm version ID binds the diagnostic configuration digest");
assert.match(main, /plugin_config_data\s*=\s*local\.callback_edge_diagnostic_config == null \? null : base64encode\(local\.callback_edge_diagnostic_config\)/, "the diagnostic configuration remains absent by default and explicit when reviewed");
